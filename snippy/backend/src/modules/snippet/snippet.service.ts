 ...
 */
const PROTECTED_SNIPPET_FIELDS = [
  'snippetId',
  'auth0Id',
  'shortId',
  'parentShortId',
...
// Create Snippet
export async function createSnippetHandler(
  payload: ServicePayload<CreateSnippetRequest>
...
// Fork Snippet
export async function forkSnippetHandler(
...
// Update Snippet
export async function updateSnippetHandler(
  payload: ServicePayload<UpdateSnippetRequest, { snippetId: string }>
): Promise<ServiceResponse<SnippetDTO>> {
  try {
    const auth0Id = payload.auth?.payload?.sub;
    if (!auth0Id) {
    ...
    return await executeInTransaction(async (t) => {
      ...
      await Promise.all(
        patchFiles.map(async (snippetFile) => {
...
export async function updateSnippetViewCountHandler(
  payload: ServicePayload<unknown, { snippetId: string }>
...
// Delete Snippet
export async function deleteSnippetHandler(
  payload: ServicePayload<unknown, { snippetId: string }>
...
export async function uploadSnippetSnapshotHandler(
  ...
  if (
    ...
    !(ALLOWED_ASSET_MIME_TYPES as readonly string[]).includes(file.mimetype)
  ) {
  ...
  try {
    ...
    await minioClient.putObject(config!.minio.bucket!, objectKey, file.buffer, file.buffer.length, {
...
export async function getSnippetByShortIdHandler(
...
export async function getUserPublicSnippetsHandler(
...
export async function getMySnippetsHandler(
