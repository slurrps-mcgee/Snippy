...
export async function deleteSnippet(snippetId: string, transaction?: Transaction): Promise<void> {
...
export async function findBySnippetId(
  ...
  return await Snippets.findByPk(snippetId, {
    include: [SnippetFiles, ...snippetWithParentInclude()],
    transaction,
  });
}
...
export async function searchSnippets(
  ...
  const where: WhereOptions = {
    isPrivate: false,
    [Op.and]: [searchCondition, ...(tagCondition ? [tagCondition] : [])],
  };
