...
export async function ensureUserHandler(
  payload: ServicePayload<EnsureUserRequest>
...
export async function updateUserHandler(
  payload: ServicePayload<UpdateUserRequest>
): Promise<ServiceResponse<UserDTO>> {
  const auth0Id = payload.auth?.payload?.sub;
  if (!auth0Id) {
    throw new CustomError('Authentication required', 401);
...
export async function deleteUserHandler(
  payload: ServicePayload<unknown>
...
export async function checkUserNameAvailabilityHandler(
  ...
  try {
    ...
    if (
      !userName ||
      userName.trim() === '' ||
      config.username.invalidUsernames.includes(userName.toLowerCase())
    ) {
