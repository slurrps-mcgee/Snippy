...
const usernameRetryPolicy = retry(
  handleType(
    ...
    (err: any) =>
      err.name === 'SequelizeUniqueConstraintError' &&
      (err.fields?.userName ||
        err.message?.includes('user_name') ||
...
usernameRetryPolicy.onFailure(() => {
  console.error(
    'All username generation attempts exhausted, falling back to timestamp-based username'
  );
});
...
export { defaultPolicy, shortIdRetryPolicy, usernameRetryPolicy, dbConnectionPolicy };
