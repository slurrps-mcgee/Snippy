...
| MinIO / assets unavailable | `ENABLE_MINIO` false, MinIO not running, or MinIO process exited | Give the `minio` service `restart: unless-stopped` (Portainer). After a connection failure the API latches MinIO off until **API restart**; the SPA probes `GET /api/v1/health` and latches Assets / Profile Image / snapshot-on-save until you restart the stack and reload |
| Portainer: `container minio is unhealthy` / failed to start | Chainguard MinIO runs as uid 65532 and cannot write a root-owned volume; or the healthcheck used `grep`/`curl` which are not in the image | Use the compose from this repo: `user: "0"` plus the bash `/dev/tcp` healthcheck. `MINIO_ROOT_PASSWORD` must be at least 8 characters. Redeploy the stack; if you previously ran `minio/minio` as root on the same volume, `user: "0"` is enough—do not keep a `grep`/curl probe |
